// User Dashboard — customer-facing panel

function UserDashboard({ user, onLogout, onOrderOpen }) {
  const { t, lang } = useT();
  ...
  return (
    <section className="border-b border-ink">
      ...
      <div className="px-6 md:px-12">
        <Tabs tabs={tabIds.map((id, i) => ({id: i, label: id}))} active={tab} onChange={setTab} />
      </div>
      ...
      <div className="px-6 md:px-12 py-8 min-h-[60vh]">
        <PageFade k={tab}>
          ...
          {tab === 3 && <UserAddresses />}
          {tab === 4 && <UserWishlist />}
          {tab === 5 && <UserSettingsTab onLogout={onLogout} />}
        </PageFade>
      </div>
    </section>
  );
...
function KPI({ label, value, trend, border }) {
  return (
...
function UserOverview({ onOrderOpen }) {
  const { t, lang } = useT();
    ...
    <div className="grid grid-cols-12 gap-6">
      <div className="col-span-12 lg:col-span-8 space-y-6">
        <div className="border border-ink">
          ...
          <div>
            {recent.map((o) => <OrderRow key={o.id} o={o} onOpen={onOrderOpen} />)}
          </div>
        </div>
      ...
      <div className="col-span-12 lg:col-span-4 space-y-6">
        <div className="border border-ink p-6 bg-ink text-paper">
          ...
          <p className="text-sm text-paper/70 mb-5">{lang === "es" ? "1% de cada pedido vuelve a tu cuenta. Aplicable en la caja." : "1% of every order returns to your account. Applies at checkout."}</p>
...
function OrderRow({ o, onOpen }) {
  ...
  return (
    <div className="grid grid-cols-12 gap-4 px-5 py-4 border-t border-ink/15 first:border-t-0 items-center hover:bg-chalk">
      ...
      <div className="col-span-6 md:col-span-1 display text-xl">{fmtMoney(o.total, "EUR", lang)}</div>
...
function UserOrders({ onOrderOpen }) {
  const { t } = useT();
  return (
    <div className="border border-ink">
      <div className="px-5 py-4 border-b border-ink flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Btn kind="outline" size="sm" icon={Icons.Filter}>Filter</Btn>
          <Btn kind="outline" size="sm" icon={Icons.Download}>Export</Btn>
        </div>
      </div>
    ...
    <div className="grid grid-cols-12 gap-6">
      <div className="col-span-12 lg:col-span-4">
        <div className="border border-ink p-6">
          ...
          <Btn kind="outline" size="sm" full icon={Icons.Upload}>{lang === "es" ? "Subir retrato" : "Upload portrait"}</Btn>
        </div>
      </div>
      <div className="col-span-12 lg:col-span-8 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Field label={lang === "es" ? "Nombre" : "First name"} value={f.first} onChange={set("first")} />
          <Field label={lang === "es" ? "Apellido" : "Last name"} value={f.last} onChange={set("last")} />
        </div>
        <Field label={t.auth.email} value={f.email} onChange={set("email")} icon={Icons.Mail} />
        <Field label={lang === "es" ? "Teléfono" : "Phone"} value={f.phone} onChange={set("phone")} icon={Icons.Phone} />
        <FieldArea label="Bio" value={f.bio} onChange={set("bio")} />
        <div className="flex justify-end gap-2">
          <Btn kind="ghost">{t.common.cancel}</Btn>
          <Btn kind="solid" icon={Icons.Save}>{t.user.saveChanges}</Btn>
        </div>
      </div>
    </div>
  );
...
function UserAddresses() {
  ...
  const addrs = [
    ...
    { label: lang === "es" ? "Atelier" : "Atelier", line1: "Casa MONOLITH", line2: "Rua do Século 7, Lisboa", phone: "+351 21 397 4400" },
  ];
  return (
          ...
          <div key={p.id} className="border border-ink">
            ...
            <div className="p-3 border-t border-ink">
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <Icons.Heart size={14} className="text-rust" />
              </div>
            </div>
          </div>
        ))}
...
function UserSettingsTab({ onLogout }) {
  const { t, lang } = useT();
    ...
    <div className="grid grid-cols-12 gap-6">
      <div className="col-span-12 lg:col-span-7 space-y-6">
        <div className="border border-ink p-6">
          ...
          {[
            { k: "emails", l: lang === "es" ? "Correos del atelier" : "Atelier emails" },
            { k: "sms",    l: "SMS · drops & shipping" },
            { k: "drops",  l: lang === "es" ? "Avisos de lanzamientos" : "Drop notifications" },
          ].map((x) => (
        ...
        <div className="border border-ink p-6">
          ...
          <div className="space-y-3 text-sm">
            <Field label={lang === "es" ? "Contraseña actual" : "Current password"} type="password" placeholder="••••••••" icon={Icons.Lock} />
            <Field label={lang === "es" ? "Nueva contraseña" : "New password"} type="password" placeholder="••••••••" icon={Icons.Lock} />
          </div>
      ...
      <div className="col-span-12 lg:col-span-5 space-y-6">
        <div className="border border-ink p-6">
          ...
          <p className="text-sm text-ink/60 mb-5">{lang === "es" ? "Para esta cuenta — la tienda se mostrará así." : "Per-account preference — the storefront will use these."}</p>
        ...
        <div className="border border-ink p-6 bg-chalk">
          ...
          <Btn kind="outline" icon={Icons.LogOut} onClick={onLogout}>{t.nav.logout}</Btn>
        </div>
      </div>
    </div>
  );
