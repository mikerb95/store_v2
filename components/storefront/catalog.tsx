...
import { Filter, Eye, ShoppingBag, Heart, X } from 'lucide-react'
import { useApp } from '@/context/app-context'
import { PRODUCTS } from '@/lib/data'
import type { Product } from '@/lib/data'
import { T } from '@/lib/translations'
import { Btn, Tag, StripePlaceholder } from '@/components/ui/primitives'
import { fmtMoney, totalStock } from '@/lib/utils'

type Filters = { category: string[]; size: string[]; color: string[]; material: string[]; price: string[] }

function FiltersRail({ filters, setFilters, onClear }: { filters: Filters; setFilters: React.Dispatch<React.SetStateAction<Filters>>; onClear: () => void }) {
  const { t } = useApp()
  ...
  const Group = ({ title, items, k }: { title: string; items: { label: string; value: string }[]; k: keyof Filters }) => (
    ...
    <aside className="sticky top-[57px] self-start">
      <div className="border border-ink bg-paper">
        <div className="flex items-center justify-between px-5 py-4 border-b border-ink">
          <div className="display text-2xl flex items-center gap-2"><Filter size={16} /> {t.catalog.filters}</div>
        ...
        <div className="px-5 pb-5">
          <Group title={t.catalog.category} items={t.catalog.categories.map((c: string, i: number) => ({ label: c, value: T.en.catalog.categories[i] }))} k="category" />
          <Group title={t.catalog.size} items={t.catalog.sizes.map((s: string) => ({ label: s, value: s }))} k="size" />
          <Group title={t.catalog.color} items={t.catalog.colors.map((c: string, i: number) => ({ label: c, value: T.en.catalog.colors[i] }))} k="color" />
          <Group title={t.catalog.material} items={t.catalog.materials.map((m: string, i: number) => ({ label: m, value: T.en.catalog.materials[i] }))} k="material" />
...
function ProductCard({ p, onQuick }: { p: Product; onQuick: (p: Product) => void }) {
  const { t, lang } = useApp()
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          <Btn kind="solid" size="sm" full icon={Eye} onClick={() => onQuick(p)}>{t.catalog.quickView}</Btn>
        </motion.div>
      </div>
      <div className="border-t border-ink p-4 flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="display text-xl">{p.name[lang as 'en' | 'es']}</h3>
        ...
        <div className="text-right">
          <div className="display text-2xl">{fmtMoney(p.price, 'EUR', lang as 'en' | 'es')}</div>
        </div>
      </div>
    </motion.div>
  )
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              ...
              <div className="p-8 md:p-12 flex flex-col">
                ...
                <h2 className="display text-5xl md:text-6xl mb-3">{product.name[lang as 'en' | 'es']}</h2>
                <div className="display text-3xl mb-6">{fmtMoney(product.price, 'EUR', lang as 'en' | 'es')}</div>
                ...
                <div className="border-t border-ink/20 pt-5 mb-5">
                  ...
                  <div className="flex gap-2">
                    {(['XS', 'S', 'M', 'L', 'XL'] as const).map((s) => {
                      const avail = (product.stock[s] || 0) > 0
                      const on = size === s
                      return (
                        <button
                          key={s}
                ...
                <div className="mt-auto flex gap-3">
                  <Btn kind="solid" size="lg" full disabled={soldOut || !size} icon={ShoppingBag}>
                    {soldOut ? t.catalog.sold : t.catalog.addBag}
                  </Btn>
                  <Btn kind="outline" size="lg" icon={Heart} />
                </div>
              </div>
            </div>
          </motion.div>
...
export function Catalog({ catalogRef }: { catalogRef?: React.RefObject<HTMLElement | null> }) {
  const { t, lang } = useApp()
  const [filters, setFilters] = React.useState<Filters>({ category: [], size: [], color: [], material: [], price: [] })
  const [sort, setSort] = React.useState(0)
  const [quick, setQuick] = React.useState<Product | null>(null)

  const inPrice = (p: Product, ranges: string[]) => {
  ...
  return (
    <section ref={catalogRef} id="catalog" className="bg-paper">
        ...
        <div className="flex items-end justify-between gap-6 flex-wrap">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="border border-ink h-11 flex items-center">
              ...
              {t.catalog.sortOptions.map((opt: string, i: number) => (
