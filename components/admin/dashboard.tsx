...
import { PRODUCTS, LIVE_ORDERS, REVENUE_SERIES, TRAFFIC_SERIES } from '@/lib/data'
import { Sparkline, BarChart, StripePlaceholder, Btn } from '@/components/ui/primitives'
    ...
    <div>
      ...
      <div className="px-6 md:px-12 py-10 border-b border-ink grid grid-cols-12 gap-6">
        <div className="col-span-12 lg:col-span-8">
          <div className="mono text-[10px] uppercase tracking-[0.3em] text-ink/60 mb-3">
            {t.admin.title} · {new Date().toLocaleDateString(lang === 'es' ? 'es-ES' : 'en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 border-b border-ink">
        {kpis.map((k, i) => (
          <div key={k.k} className={`p-5 ${i > 0 ? 'border-l border-ink' : ''}`}>
            <div className="mono text-[10px] uppercase tracking-[0.25em] text-ink/60">{(t.admin.kpis as Record<string, string>)[k.k]}</div>
      ...
      <div className="grid grid-cols-12 border-b border-ink">
        <div className="col-span-12 lg:col-span-8 p-6 border-r border-ink">
          ...
          <div className="h-56"><RevenueChart data={REVENUE_SERIES} /></div>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="display text-3xl mt-1 flex items-center gap-3">
                Live feed <span className="w-2 h-2 bg-rust animate-pulse" />
              </div>
            </div>
            <Btn kind="outline" size="sm" iconRight={ArrowUpRight}>{t.admin.view}</Btn>
          </div>
          <div className="border border-ink">
            {LIVE_ORDERS.map((o) => (
              <div key={o.id} className="grid grid-cols-12 items-center gap-3 px-4 py-3 border-t border-ink/15 first:border-t-0 hover:bg-chalk">
                ...
                <div className="col-span-3 display text-xl text-right">{fmtMoney(o.amount, 'EUR', lang as 'en' | 'es')}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          {PRODUCTS.slice(0, 5).map((p, i) => (
            <div key={p.id} className="grid grid-cols-12 items-center gap-3 py-3 border-t border-ink/15 first:border-t-0">
              ...
              <div className="col-span-5">
                <div className="text-sm">{p.name[lang as 'en' | 'es']}</div>
              ...
              <div className="col-span-2 display text-xl text-right">{fmtMoney(p.price, 'EUR', lang as 'en' | 'es')}</div>
            </div>
          ))}
        </div>
      </div>
